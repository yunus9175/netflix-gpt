export const netflixLogo =
  "https://cdn.cookielaw.org/logos/dd6b162f-1a32-456a-9cfe-897231c7763c/4345ea78-053c-46d2-b11e-09adaef973dc/Netflix_Logo_PMS.png";

export const UserLogo =
  "data:image/png;base64,";

export const LogoutIcon = "https://www.svgrepo.com/show/21304/logout.svg";

export const PhotoURL =
  "https://img.freepik.com/free-vector/smiling-young-man-glasses_1308-174702.jpg?size=626&ext=jpg";

export const BackgroundImg =
  "https://assets.nflxext.com/ffe/siteui/vlv3/f272782d-cf96-4988-a675-6db2afd165e0/web/IN-en-20241008-TRIFECTA-perspective_b28b640f-cee0-426b-ac3a-7c000d3b41b7_small.jpg";

export const API_key = "7912ae124fca63c91f2d56f0c42c303c";

export const getAPIOptions = (method = "GET") => ({
  method: method,
  headers: {
    accept: "application/json",
    Authorization:
      "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI3OTEyYWUxMjRmY2E2M2M5MWYyZDU2ZjBjNDJjMzAzYyIsIm5iZiI6MTcyOTM1MzgwNy45MjczNjYsInN1YiI6IjY3MTNiNWMwYzZlMzA0MDk2MTk1ZGJiYiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.fltRJvAEDdgGJ-B7lOkDyKdVDDYhZNZQzYlV36aL5XE",
  },
});
